...
void copyDirectory(struct DirectoryList* list, const char* source, const char* dest) {
    if (strcmp(source, dest) == 0) {
        return; // source와 dest가 같으면 복사하지 않음
    }
    ...
    while (current != NULL) {
        ...
        if (strncmp(current->name, source, sourceLen) == 0 && (nameLen == sourceLen || current->name[sourceLen] == '/')) {
            ...
            if (strlen(subDir) > 0) {
                ...
                // 복사 대상 디렉토리에 이미 존재하는지 확인
                int isDuplicate = 0;
                struct DirectoryNode* checkNode = list->head;
                while (checkNode != NULL) {
                    if (strcmp(checkNode->name, subDirDest) == 0) {
                        isDuplicate = 1;
                        break;
                    }
